...
namespace DiagramDesigner
{
    ...
    {
        public static SelectionService Instance = new SelectionService();
        private List<DesignerCanvas> designerCanvas { get => DesignerCanvas.List; }
        ...
        {
            ClearSelection();
            designerCanvas.ForEach(x => CurrentSelection.AddRange(x.Children.OfType<ISelectable>()));
            CurrentSelection.ForEach(item => item.IsSelected = true);
        }

        internal List<IGroupable> GetGroupMembers(IGroupable item)
        {
            List<IGroupable> list = new List<IGroupable>();
            designerCanvas.ForEach(x => list.AddRange(x.Children.OfType<IGroupable>()));
